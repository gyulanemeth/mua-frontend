...
<template >
  <v-layout class="d-flex flex-wrap w-75">
          <v-col class="pt-3">
            <h3 class="font-weight-bold text-error">{{ $t(`muaAuth.accountAdminSettings.delete${props.data.role}Label`) }}</h3>
                    ...
                    <v-banner-text>
                       <h3 class="font-weight-bold">{{ $t('muaAuth.accountAdminSettings.noteHeader') }}</h3>
                        <v-list-item color="error">
                            ...
                            <v-card-text class="pa-0 ma-0 text-body-1">{{ $t(`muaAuth.accountAdminSettings.note${props.data.role}Point1`) }}</v-card-text>
                        </v-list-item>
                        <v-list-item color="error">
                            <template v-slot:prepend>
                                <v-icon icon="mdi-circle-small"></v-icon>
                            </template>
                            <v-card-text class="pa-0 ma-0 text-body-1">{{ $t(`muaAuth.accountAdminSettings.note${props.data.role}Point2`) }}</v-card-text>
                        </v-list-item>
                        <v-list-item v-if="props.data.role=== 'admin'" color="error">
                            <template v-slot:prepend>
                                <v-icon icon="mdi-circle-small"></v-icon>
                            </template>
                            <v-card-text class="pa-0 ma-0 text-body-1">{{ $t(`muaAuth.accountAdminSettings.note${props.data.role}Point3`) }}</v-card-text>
                        </v-list-item>
                        <v-list-item v-if="props.data.role=== 'admin'" color="error">
                            <template v-slot:prepend>
                                <v-icon icon="mdi-circle-small"></v-icon>
                            </template>
                            <v-card-text class="pa-0 ma-0 text-body-1">{{ $t(`muaAuth.accountAdminSettings.note${props.data.role}Point4`) }}</v-card-text>
                        </v-list-item>
                        <h3 class="font-weight-bold">{{ $t(`muaAuth.accountAdminSettings.noteFooter`) }}</h3>
                </v-banner-text>
            </v-banner>

...
<template>
    <v-form class="d-flex flex-column justify-center align-center h-screen">
        ...
        <v-card class="ma-2 pa-2  rounded-xl  elevation-2" width="80%" max-width="600px">
            <v-card-text align="center">
                <h6 class="text-h6">{{ $t('muaAuth.userLoginAndResetForm.resetHeader') }} </h6>
                ...
                <div v-if="route.name === 'accounts-forgot-password-reset'">
                    ...
                    <div v-if="cb">
                        <v-text-field hide-details data-test-id="loginAndResetForm-newPasswordField" density="compact"
                            ...
                            :label="$t('muaAuth.userLoginAndResetForm.newPasswordLabel')" type="password"
                            :placeholder="data.password || $t('muaAuth.userLoginAndResetForm.newPasswordPlaceholder')"
                            :value="data.password"
                        ...
                        <v-text-field hide-details density="compact" class=" my-5 rounded"
                            ...
                            :label="$t('muaAuth.userLoginAndResetForm.confirmNewPasswordLabel')" type="password"
                            :placeholder="data.confirmPassword || $t('muaAuth.userLoginAndResetForm.confirmNewPasswordPlaceholder')"
                ...
                <div v-if="route.name !== 'accounts-forgot-password-reset'">
                    <div v-if="cb !== 'reset'">
                        ...
                        <v-btn color="info" :disabled="!checkbox || !data.account || !data.email " data-test-id="loginAndResetForm-forgotPasswordBtn"
                            ...
                            {{ !processing ? $t('muaAuth.userLoginAndResetForm.resetBtnText') : '' }}
                    ...
                    <p v-if="cb === 'reset'" data-test-id="loginAndResetForm-forgotPasswordCb" class="mt-4">
                        {{ $t('muaAuth.userLoginAndResetForm.cb.resetMessage') }}</p>
                </div>
            </v-card-text>

...
<template>
    <v-form class="d-flex flex-column justify-center align-center h-screen">
        ...
        <v-card class="ma-2 pa-2  rounded-xl  elevation-2" width="80%" max-width="600px">
            <v-card-text align="center">
                <h6 class="text-h6">{{ $t('muaAuth.adminForgotPasswordForm.header') }}</h6>
                ...
                <v-text-field hide-details data-test-id="forgotPassword-emailField" density="compact"
                    ...
                    :label="$t('muaAuth.adminForgotPasswordForm.emailLabel')" type="email" :disabled="cb"
                    :placeholder="data || $t('muaAuth.adminForgotPasswordForm.emailPlaceHolder')" :value="data"
                    @update:modelValue="res => data = res.replace(/[^a-z0-9+@ \.,_-]/gim, '')" required />
                <v-checkbox v-if="!cb" :label="$t('muaAuth.adminForgotPasswordForm.checkboxLabel')" color="info" v-model="checkbox"
                ...
                <div v-if="!cb">
                    <v-btn data-test-id="forgotPassword-submitBtn" :disabled="!checkbox || !data" color="info"
                        ...
                        {{ !processing ? $t('muaAuth.adminForgotPasswordForm.submitBtn') : '' }}

...
<template>
    ...
    <v-dialog v-model="dialogShown"  scrollable tabindex="1"
        ...
        <v-card :width="!$vuetify.display.mdAndUp? '100%': '50%'" max-width="800" class="ma-auto d-flex flex-column justify-center">
            ...
            <v-card-text align="start">
                <v-col align="center" class="pb-10">
                    <v-toolbar-title class="font-weight-bold text-error">{{ $t('mua.adminDeleteAccount.header')}}
                ...
                <v-card-actions>
                    ...
                    <v-btn color="error" data-test-id="deleteAccount-submitBtn"
                        @click="$emit('deleteEventHandler', { id: props.data._id, password });resetForm()">{{ $t('mua.adminDeleteAccount.submitBtn')}}</v-btn>
                </v-card-actions>
            </v-card-text>
        </v-card>
    </v-dialog>

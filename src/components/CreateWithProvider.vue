    ...
    if (popup) {
      const interval = setInterval(async () => {
        try {
          ...
          if (!popup.closed && (loginToken || failedQuery || userData)) {
            clearInterval(interval)
            popup.close()
            if (userData) {
              const data = jwtDecode(userData)
...
<template>
  <v-card-text align="left">
    ...
    <v-btn v-if="githubProvider" block @click="submitCreateWithProvider('github')" class="pa-2 border mt-2" variant="text">
      ...
      <span class="mx-2 text-caption font-weight-medium">{{ $t('mua.createWithProvider.githubBtn') }}</span>
    </v-btn>
  </v-card-text>
</template>

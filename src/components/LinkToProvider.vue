...
async function submitLinkToProvider (provider) {
  ...
  if (res.redirectUrl) {
    ...
    const handleMessage = async (event) => {
      if (event.data.type === 'provider-link-token') {
    ...
    if (popup) {
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          if (!popup.closed && (success || failed)) {
            popup.opener.postMessage({
              type: 'provider-link-token',
              success,
              failed
            }, '*')
            popup.close()
...
<template>
    <div class="mb-5">
        <p class="text-body-1 font-weight-bold ">Connect Account</p>
        <v-divider />
        <v-card-text align="left">
            <v-btn  @click="submitLinkToProvider('google')" class="pa-2 border mr-3" variant="text">
                ...
                <span class="mx-2 text-caption font-weight-medium">Connect to Google</span>
            </v-btn>
            <v-btn  @click="submitLinkToProvider('microsoft')" class="pa-2 border mr-3"
                variant="text">
                <span>
                ...
                <span class="mx-2 text-caption font-weight-medium">Connect to Microsoft</span>
            </v-btn>
            <v-btn  @click="submitLinkToProvider('github')" class="pa-2 border"
                variant="text">
                <span>
                ...
                <span class="mx-2 text-caption font-weight-medium">Connect to Github</span>
            </v-btn>
        </v-card-text>
    </div>
</template>

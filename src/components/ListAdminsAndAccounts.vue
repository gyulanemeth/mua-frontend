...
import Dialog from '../components/CreateAdminDialog.vue'
import DeleteMyAccount from './DeleteAdminAccount.vue'
...
<div class="elevation-2 mx-10 pa-3 pt-0 rounded">
    <v-layout class="d-flex flex-wrap">
        <v-col cols="2" class="pt-3 d-flex align-center">
            <p class="text-h6">{{route.name === 'system-admins'?  $t('muaAuth.listAdminsAndAccounts.header.admin') : $t('muaAuth.listAdminsAndAccounts.header.account')}} </p>
        </v-col>
        <v-spacer />
        ...
        <v-col cols="2" class="pt-3 d-flex align-center">
          <v-btn variant="outlined" color="info" @click="createAccountDialog.show()">
                {{ route.name === 'system-admins'? $t('muaAuth.listAdminsAndAccounts.createAdminBtn'): $t('muaAuth.listAdminsAndAccounts.createAccountBtn') }}
            </v-btn>
        ...
        <v-col cols="2" class="d-flex align-center ma-0 pa-0">
           <div >
              ...
              <v-card-text class="pt-0">
                <p class="font-weight-medium" >{{ route.name === 'system-admins'? $t('muaAuth.emptyList.inviteFirstAdmin') : $t('muaAuth.emptyList.addFirstAccount')}} </p>
              </v-card-text>
            </div>
        </v-col>
    </v-layout>
    ...
    <v-layout v-else-if="props.items.length === 0" :class="`ma-auto d-flex flex-wrap pa-4 ${filter.length > 0 ? 'h-75':'h-50'}`">
          <v-card class="ma-auto align-self-start elevation-0 text-center" :min-width="filter.length === 0 ? route.name === 'system-admins'?  320 : 280: 400">
            <v-avatar size="150">
            ...
            <v-row class="mt-2" >
               ...
               <v-col cols="10" class="pt-4 ml-0 pl-0">
                 <h3  v-if="filter.length === 0">{{ $t('muaAuth.emptyList.NoElementsYet', {name: route.name === 'system-admins'?  $t('muaAuth.listAdminsAndAccounts.header.admin') : $t('muaAuth.listAdminsAndAccounts.header.account')}) }}</h3>
    ...
    <v-layout class="d-flex flex-wrap">
        <v-table fixed-header class="w-100">
            <thead>
                <tr>
                    <th class="text-left">
                        {{$t('muaAuth.listAdminsAndAccounts.tableHeader.nameLabel')}}
                    </th>
                    <th v-if="route.name === 'system-admins'" class="text-left">
                        {{$t('muaAuth.listAdminsAndAccounts.tableHeader.emailLabel')}}
                    </th>
                    <th v-else class="text-left">
                        {{$t('muaAuth.listAdminsAndAccounts.tableHeader.urlFriendlyName')}}
                    </th>
                    <th class="text-left">
                        {{$t('muaAuth.listAdminsAndAccounts.tableHeader.lastEditedLabel')}}
                    </th>
                    <th class="text-left">
                        {{$t('muaAuth.listAdminsAndAccounts.tableHeader.creationDateLabel')}}
                    </th>
                    <th>
                    </th>
                </tr>
            ...
            <tbody>
                ...
                <tr v-for="(item, i) in props.items" :key="item._id">
                    ...
                    <td v-if="route.name === 'system-admins'" :data-test-id="`tableList-${i}-deleteBtn`" class="text-right">
                        <v-tooltip :text="$t('muaAuth.listAdminsAndAccounts.resendMessage')">
    ...
    <v-layout class="d-flex flex-wrap justify-center align-center">
        <v-spacer />
        <p class="ma-2">
          {{$t('muaAuth.listAdminsAndAccounts.tableFooter.label')}}
        </p>
        <p class="ma-2">

...
const projectId = ref(props.data.projectId)
const permission = ref(props.data.permission)
...
defineExpose({
    show,
    hide: () => { dialogShown.value = false; role.value = props.data.role, projectId.value = props.data.projectId, permission.value = props.data.permission  }
})
...
<template>
    ...
    <v-dialog v-model="dialogShown" tabindex="-1"
        @keydown.enter="props.roles && $emit('updateRoleEventHandler', { id: props.data._id, role, projectId, permission }); dialogShown = false"
        ...
        <v-card :width="!$vuetify.display.mdAndUp ? '100%' : '50%'" max-width="800" class="ma-auto">
            <v-container class="d-flex flex-column justify-center">
                ...
                <v-card-text align="start">
                    ...
                    <v-row v-if="role === 'client'" align="center">
                        <v-col>
                            <p class="font-weight-bold">{{ $t('mua.userCard.projectLabel') }}</p>
                        </v-col>
                        <v-col cols="7">
                            <v-select hide-details data-test-id="userProfile-selectRole" v-model="projectId"
                    ...
                    <v-row v-if="role === 'client'" align="center">
                        ...
                        <v-col cols="7">
                            <v-select hide-details data-test-id="userProfile-selectRole" v-model="permission"
                                :disabled="!props.permissions" density="compact" color="primary" class="my-5 rounded"
                                variant="solo" :items="props.permissions"
                                name="projectId" />
                        </v-col>
                    </v-row>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="primary" v-if="props.roles" data-test-id="userProfile-submitBtn"
                        @click="$emit('updateRoleEventHandler', { id: props.data._id, role, projectId, permission }); dialogShown = false">{{
